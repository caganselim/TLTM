...
def transform_instance_annotations(
    ...
    bbox = BoxMode.convert(annotation["bbox"], BoxMode.XYWH_ABS, BoxMode.XYXY_ABS)
    ...
    #assert cat_id >= 0, "lower idx"
    #assert cat_id < 40, "upper idx"
    ...
    if "segmentation" in annotation:
        ...
        if isinstance(segm, list):
            ...
            assert tuple(mask.shape[:2]) == image_size, f"{tuple(mask.shape[:2])} != {image_size}"
            annotation["segmentation"] = mask
        else:
            raise ValueError(
...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    try:
        track_id = [int(obj["track_id"]) for obj in annos]
        track_id = torch.tensor(track_id, dtype=torch.int64)
    except:
        num_instances = len(boxes)
        track_id = torch.arange(num_instances, dtype=torch.int64)

    target.gt_track_id = track_id
