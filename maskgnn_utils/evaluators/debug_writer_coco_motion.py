...
class DebugWriterCocoMotion(DatasetEvaluator):
    """
...
.
    ...
    def save_visuals(self, input, prev_preds=None, prev_pred_ids=None, preds=None, pred_ids=None, debug_dict=None):
        """
        ...
        filename_0 =  input["file_name"]
        self._logger.warning(f"[DebugWriter] Saving visualizations - {filename_0}.")

        tokens = filename_0.split('/')
        ...
        image_0 = input["debug_image_0"][:,:,::-1]
        image_1 = input["debug_image_1"][:,:,::-1]
        ...
        prev_pred_ids = [id.item() for id in prev_pred_ids]

        im0_vis = visualizer.draw_preds_with_tracking_ids(prev_preds.to("cpu"), prev_pred_ids)
        prev_vis = im0_vis.get_image()

        pred_ids = [id.item() for id in pred_ids]
        visualizer1 = InstanceVisualizer(image_1, metadata=metadata)
        im1_vis = visualizer1.draw_preds_with_tracking_ids(preds.to("cpu"), pred_ids)
        vis = im1_vis.get_image()
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            pred_instances_0 = output['instances_0']
            tracking_ids_0 = pred_instances_0.tracking_id
            tracking_ids_0 = [id + 1 for id in tracking_ids_0]

            pred_instances_1 = output['instances_1']
            tracking_ids_1 = pred_instances_0.tracking_id
            tracking_ids_1 = [id + 1 for id in tracking_ids_1]

            debug_dict = output["debug_dict"]

            self.save_visuals(input=input, prev_preds=pred_instances_0, prev_pred_ids=tracking_ids_0,
                              preds=pred_instances_1, pred_ids=tracking_ids_1, debug_dict=debug_dict)
