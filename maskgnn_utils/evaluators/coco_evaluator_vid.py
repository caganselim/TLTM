...
from .coco_helper import convert_to_coco_json
...
class COCOEvaluatorVID(DatasetEvaluator):
    """
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            #     output['instances'][i].pred_classes += 1
...
def instances_to_coco_json(instances, img_id):
    ...
    for k in range(num_instance):
        result = {
            "image_id": img_id,
            "category_id": classes[k], #todo can be problematic :)
            "bbox": boxes[k],
            "score": scores[k],
        }
        if has_mask:
