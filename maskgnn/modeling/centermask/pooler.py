...
def _img_area(instance):
    ...
    if instance.has("pred_classes"):
        ...
        tmp_len = len(instance.pred_classes)

    else:
        device = instance.gt_classes.device
        tmp_len = len(instance.gt_classes)

    image_size = instance.image_size
    area = torch.as_tensor(image_size[0] * image_size[1], dtype=torch.float, device=device)
    tmp = torch.zeros((tmp_len, 1), dtype=torch.float, device=device)

    return (area + tmp).squeeze(1)


def assign_boxes_to_levels_by_ratio(instances, min_level, max_level, target_key="proposal_boxes"):
    """
    ...
    if target_key == "gt_boxes":
        box_lists = [x.gt_boxes for x in instances]
    elif target_key == "pred_boxes":
        box_lists = [x.pred_boxes for x in instances]
    elif target_key == "proposal_boxes":
...
class ROIPooler(nn.Module):
    ...
    def forward(self, x, instances, target_key="proposal_boxes"):
        """
        Args:
        ...
        if self.assign_crit == "ratio":
            level_assignments = assign_boxes_to_levels_by_ratio(
                instances, self.min_level, self.max_level, target_key
            )
        else: #default
