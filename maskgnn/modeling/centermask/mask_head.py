...
def mask_rcnn_loss(pred_mask_logits, instances, maskiou_on):
    ...
    for instances_per_image in instances:
        ...
        if maskiou_on:
            ...
            bitmasks = copy.deepcopy(instances_per_image.gt_masks.tensor)
            boxes = torch.floor(instances_per_image.proposal_boxes.tensor).to(torch.int64)
            cropped_mask = []
            for idx in range(bitmasks.shape[0]):
                xmin, ymin, xmax, ymax = boxes[idx,0], boxes[idx,1], boxes[idx,2], boxes[idx,3]

                bitmask = bitmasks[idx, ymin:ymax, xmin:xmax]
                cropped_mask.append(bitmask.sum())

            cropped_mask = torch.tensor(cropped_mask).to(device=pred_mask_logits.device)
            gt_masks_areas = instances_per_image.gt_masks.tensor.sum(2).sum(1)

            mask_ratios.append(
                (cropped_mask.to(torch.float32) / gt_masks_areas.to(torch.float32))
...
def mask_rcnn_inference(pred_mask_logits, pred_instances):
    ...
    for prob, instances in zip(mask_probs_pred, pred_instances):
        ...
        instances.pred_masks = prob  # (1, Hmask, Wmask) valued in btw 0-1, not binary.
