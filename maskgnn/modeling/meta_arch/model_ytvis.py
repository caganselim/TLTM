...
class CenterMaskGNN(nn.Module):
    ...
    def _tracker_update(self, dets):
        ...
        if (self.prev_hits is not None) and self.forget_mode:
            print(self.prev_hits)
            remove_idxs = self.prev_hits > -self.num_misses_to_forget

            if self.is_gnn_on:
                self.prev_obj_feats = self.prev_obj_feats[remove_idxs]
            self.prev_bboxes = self.prev_bboxes[remove_idxs]
            self.prev_det_labels = self.prev_det_labels[remove_idxs]
            self.prev_masks = self.prev_masks[remove_idxs]
            self.prev_hits = self.prev_hits[remove_idxs]
        ...
        if self.is_first or (not self.is_first and self.prev_bboxes is None):
            ...
            self.prev_hits = torch.ones(det_num_objs, dtype=torch.int32)
            det_obj_ids = np.arange(det_num_objs, dtype=np.int32)
            debug_dict = None
            self.id_counter += det_num_objs
            ...
            """
            We assembly match_scores: (num_dets, prev_dets + 1); where +1 stands for a new object case.
            """
            ...
            if self.is_gnn_on:
                ...
                match_score = torch.zeros(len(det), self.prev_bboxes.shape[0] + 1).to(self.device)
            ...
            prev_bbox_img_n = [self.prev_bboxes.size(0)]
            ...
            for idx, match_id in enumerate(match_ids):
                if match_id == 0:
                    ...
                    # add new object
                    det_obj_ids[idx] = self.id_counter
                    self.id_counter += 1
                    ...
                    self.prev_hits = torch.cat((self.prev_hits, torch.ones(1)), dim=0)
