...
from maskgnn.modeling.meta_arch.tracker import GenericTracker
...
class CenterMaskGNN(nn.Module):
    ...
    def _tracker_update(self, dets):
        ...
        pred_instances = dets[0]


        if self.is_gnn_on:

            prev_obj_features = self.tracker.get_obj_features()

            if prev_obj_features is not None:

                similarity_matrix = self.obj2obj_gnn.forward_test_v2(prev_obj_features, pred_instances.obj_feats)
                print("Similarity matrix: " , similarity_matrix.shape, similarity_matrix)



        dets_with_tracking_ids = self.tracker.update(pred_instances)
        dets[0]= dets_with_tracking_ids
